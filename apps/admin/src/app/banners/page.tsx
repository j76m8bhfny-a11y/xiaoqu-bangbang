...
const statusColors: Record<string, string> = {
  draft: 'default', published: 'green', offline: 'red',
};

const positionLabels: Record<string, string> = {
  home_top: '首页顶部', event_list: '事件列表', market_list: '闲置列表',
};

const linkTypeLabels: Record<string, string> = {
  event: '事件', market: '闲置', announcement: '公告', service_provider: '服务商', url: '链接', none: '无',
};
...
export default function BannersPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { message.success('Banner已创建'); setCreateModalOpen(false); createForm.resetFields(); queryClient.invalidateQueries({ queryKey: ['admin', 'banners'] }); },
    onError: () => message.error('创建失败'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, ...values }: any) => api.patch(`/admin/banners/${id}`, values),
    onSuccess: () => { message.success('Banner已更新'); setEditModalOpen(false); queryClient.invalidateQueries({ queryKey: ['admin', 'banners'] }); },
    onError: () => message.error('更新失败'),
  });

  const publishMutation = useMutation({
    mutationFn: (id: string) => api.post(`/admin/banners/${id}/publish`),
    onSuccess: () => { message.success('已发布'); queryClient.invalidateQueries({ queryKey: ['admin', 'banners'] }); },
    onError: () => message.error('操作失败'),
  });

  const offlineMutation = useMutation({
    mutationFn: (id: string) => api.post(`/admin/banners/${id}/offline`),
    onSuccess: () => { message.success('已下线'); queryClient.invalidateQueries({ queryKey: ['admin', 'banners'] }); },
    onError: () => message.error('操作失败'),
  });
  ...
  const columns: ColumnsType<any> = [
    ...
    {
      title: '链接类型', dataIndex: 'linkType', width: 80,
  ...
  return (
    <AuthGuard>
      <AdminLayout>
        ...
        <Modal title="新增Banner" open={createModalOpen} onCancel={() => setCreateModalOpen(false)}
        ...
        >
          <Form form={createForm} layout="vertical">
            ...
            <Form.Item name="position" label="位置" initialValue="home_top">
              <Select options={Object.entries(positionLabels).map(([v, l]) => ({ value: v, label: l }))} />
            </Form.Item>
            <Form.Item name="linkType" label="链接类型" initialValue="none">
              <Select options={Object.entries(linkTypeLabels).map(([v, l]) => ({ value: v, label: l }))} />
            </Form.Item>
        ...
        <Modal title="编辑Banner" open={editModalOpen} onCancel={() => setEditModalOpen(false)}
