...
import {
  Table, Card, Tag, Space, Button, Modal, Form, Input, Select, InputNumber, message,
} from 'antd';
...
const categoryLabels: Record<string, string> = {
  repair: '维修', cleaning: '保洁', lock: '开锁', home_appliance: '家电',
  moving: '搬家', pet: '宠物', other: '其他',
};

const statusLabels: Record<string, string> = {
  pending_review: '待审核', published: '已发布', offline: '已下线', rejected: '已拒绝',
};

const statusColors: Record<string, string> = {
  pending_review: 'orange', published: 'green', offline: 'default', rejected: 'red',
};

const sourceLabels: Record<string, string> = {
  platform: '平台', committee: '业委会', community: '社区',
};
...
export default function ServiceProvidersPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { message.success('服务商已创建'); setCreateModalOpen(false); createForm.resetFields(); queryClient.invalidateQueries({ queryKey: ['admin', 'service-providers'] }); },
    onError: () => message.error('创建失败'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, ...values }: any) => api.patch(`/admin/service-providers/${id}`, values),
    onSuccess: () => { message.success('服务商已更新'); setEditModalOpen(false); queryClient.invalidateQueries({ queryKey: ['admin', 'service-providers'] }); },
    onError: () => message.error('更新失败'),
  });

  const publishMutation = useMutation({
    mutationFn: (id: string) => api.post(`/admin/service-providers/${id}/publish`),
    onSuccess: () => { message.success('已发布'); queryClient.invalidateQueries({ queryKey: ['admin', 'service-providers'] }); },
    onError: () => message.error('操作失败'),
  });

  const offlineMutation = useMutation({
    mutationFn: (id: string) => api.post(`/admin/service-providers/${id}/offline`),
    onSuccess: () => { message.success('已下线'); queryClient.invalidateQueries({ queryKey: ['admin', 'service-providers'] }); },
    onError: () => message.error('操作失败'),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => { message.success('已拒绝'); setRejectModalOpen(false); setRejectReason(''); queryClient.invalidateQueries({ queryKey: ['admin', 'service-providers'] }); },
    onError: () => message.error('操作失败'),
  });
  ...
  const columns: ColumnsType<any> = [
    ...
    {
      title: '推荐来源', dataIndex: 'recommendationSource', width: 80,
    ...
    { title: '排序', dataIndex: 'sortOrder', width: 60 },
    {
      title: '操作', width: 250, render: (_, record) => (
        <Space size="small" wrap>
          ...
          {record.status === 'pending_review' && (
            <Button size="small" type="link" danger onClick={() => { setRejectId(record.id); setRejectModalOpen(true); }}>拒绝</Button>
          )}
          {record.status === 'published' && (
            <Button size="small" type="link" danger onClick={() => offlineMutation.mutate(record.id)}>下线</Button>
          )}
        </Space>
      ),
    },
  ...
  return (
    <AuthGuard>
      <AdminLayout>
        ...
        <Modal title="新增服务商" open={createModalOpen} onCancel={() => setCreateModalOpen(false)}
        ...
        <Modal title="编辑服务商" open={editModalOpen} onCancel={() => setEditModalOpen(false)}
          onOk={() => editForm.validateFields().then((v) => updateMutation.mutate({ id: editingItem.id, ...v }))}
        ...
        <Modal title="拒绝服务商" open={rejectModalOpen}
          onOk={() => { if (rejectReason) rejectMutation.mutate({ id: rejectId, reason: rejectReason }); }}
