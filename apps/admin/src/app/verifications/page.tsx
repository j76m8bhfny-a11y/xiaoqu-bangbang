...
const statusLabels: Record<string, string> = {
  pending: '待审核', approved: '已通过', rejected: '已拒绝', manual_review: '人工审核',
};

const statusColors: Record<string, string> = {
  pending: 'orange', approved: 'green', rejected: 'red', manual_review: 'blue',
};

const materialLabels: Record<string, string> = {
  property_cert: '房产证', rent_contract: '租赁合同', access_card: '门禁卡', other: '其他',
};
...
export default function VerificationsPage() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => { message.success('已通过'); queryClient.invalidateQueries({ queryKey: ['admin', 'verifications'] }); setDrawerOpen(false); },
      ...
      title: '操作', width: 160, render: (_, record) => (
        <Space size="small">
          ...
          {record.status === 'pending' || record.status === 'manual_review' ? (
            <>
              <Button size="small" type="link" onClick={() => approveMutation.mutate(record.id)}>通过</Button>
        ...
        <Drawer title="认证详情" open={drawerOpen} onClose={() => setDrawerOpen(false)} width={600}>
          {selected && detailQuery.data?.data && (
            <Descriptions column={1} bordered size="small">
              ...
              <Descriptions.Item label="用户">{detailQuery.data.data.user?.nickname}</Descriptions.Item>
              <Descriptions.Item label="材料类型">{materialLabels[detailQuery.data.data.materialType] || detailQuery.data.data.materialType}</Descriptions.Item>
              <Descriptions.Item label="状态">
                <Tag color={statusColors[detailQuery.data.data.status]}>{statusLabels[detailQuery.data.data.status]}</Tag>
              </Descriptions.Item>
              ...
              <Descriptions.Item label="脱敏材料">{detailQuery.data.data.maskedFileUrl || '-'}</Descriptions.Item>
              ...
              <Descriptions.Item label="拒绝原因">{detailQuery.data.data.rejectReason || '-'}</Descriptions.Item>
              <Descriptions.Item label="创建时间">{detailQuery.data.data.createdAt}</Descriptions.Item>
            </Descriptions>
          )}
        </Drawer>

        <Modal
          title="拒绝认证" open={rejectModalOpen}
