...
export default function SocialGroupsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { message.success('社群已创建'); setCreateModalOpen(false); createForm.resetFields(); queryClient.invalidateQueries({ queryKey: ['admin', 'social-groups'] }); },
    onError: () => message.error('创建失败'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, ...values }: any) => api.patch(`/admin/community-social-groups/${id}`, values),
    onSuccess: () => { message.success('社群已更新'); setEditModalOpen(false); queryClient.invalidateQueries({ queryKey: ['admin', 'social-groups'] }); },
    onError: () => message.error('更新失败'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/admin/community-social-groups/${id}`),
    onSuccess: () => { message.success('社群已删除'); queryClient.invalidateQueries({ queryKey: ['admin', 'social-groups'] }); },
    onError: () => message.error('删除失败'),
  });

  const visibleLabels: Record<string, string> = {
    verified_only: '仅认证用户', public: '所有人',
  };
  ...
  const columns: ColumnsType<any> = [
    ...
    {
      title: '可见性', dataIndex: 'visibleTo', width: 100,
    ...
    { title: '排序', dataIndex: 'sortOrder', width: 60 },
    {
      title: '操作', width: 180, render: (_, record) => (
        <Space size="small">
          ...
          <Button size="small" type="link" danger onClick={() => deleteMutation.mutate(record.id)}>删除</Button>
        </Space>
      ),
    },
  ];
  ...
  return (
    <AuthGuard>
      <AdminLayout>
        ...
        <Modal title="新增社群" open={createModalOpen} onCancel={() => setCreateModalOpen(false)}
        ...
        >
          <Form form={createForm} layout="vertical">
            ...
            <Form.Item name="visibleTo" label="可见性" initialValue="verified_only">
              <Select options={[{ value: 'verified_only', label: '仅认证用户' }, { value: 'public', label: '所有人' }]} />
            </Form.Item>
        ...
        <Modal title="编辑社群" open={editModalOpen} onCancel={() => setEditModalOpen(false)}
