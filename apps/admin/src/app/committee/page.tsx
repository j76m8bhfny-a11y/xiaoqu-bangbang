...
export default function CommitteePage() {
  ...
  const createMemberMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin', 'committee', 'members'] });
    },
  ...
  const approveClaimMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin', 'committee', 'claims'] });
    },
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Button
            ...
            type="link"
            danger
            onClick={() => deleteMemberMutation.mutate(record.id)}
          >
            删除
          </Button>
        </Space>
          ...
          {record.status === 'pending' && (
            <>
              <Button
                size="small"
                type="link"
                onClick={() => approveClaimMutation.mutate(record.id)}
              >
                通过
              </Button>
              <Button
      ...
      <AdminLayout>
                ...
                children: (
                  <>
                    ...
                    <Table
                      ...
                      dataSource={membersQuery.data?.data?.items ?? []}
                      loading={membersQuery.isLoading}
                      pagination={{
                        current: page,
                        pageSize,
                        total: membersQuery.data?.data?.total ?? 0,
              ...
              },
              {
                ...
                children: (
                  <Table
                    ...
                    dataSource={claimsQuery.data?.data?.items ?? []}
                    loading={claimsQuery.isLoading}
                    pagination={{
                      current: page,
                      pageSize,
                      total: claimsQuery.data?.data?.total ?? 0,
                      showSizeChanger: true,
                      onChange: (p, ps) => {
                        setPage(p);
                        setPageSize(ps);
        ...
        <Modal
          ...
          onOk={() =>
            editForm
              .validateFields()
              .then((v) => updateMemberMutation.mutate({ id: editingMember.id, ...v }))
          }
          ...
          {selectedClaim && (
            <Descriptions column={1} bordered size="small">
              ...
              <Descriptions.Item label="成员ID">
                {selectedClaim.committeeMemberId}
              </Descriptions.Item>
              ...
              <Descriptions.Item label="状态">
                <Tag color={claimStatusColors[selectedClaim.status]}>
        ...
        <Modal
          ...
          onOk={() => {
            if (rejectReason) {
              rejectClaimMutation.mutate({ id: rejectClaimId, reason: rejectReason });
            } else {
