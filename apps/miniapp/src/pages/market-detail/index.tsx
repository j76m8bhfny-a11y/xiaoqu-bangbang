...
const TRADE_TYPE_LABELS: Record<string, { label: string; color: string; bgColor: string }> = {
  ...
  [TradeType.FREE]: { label: '免费', color: '#35e89a', bgColor: '#e9fff4' },
  [TradeType.EXCHANGE]: { label: '交换', color: '#ff9f43', bgColor: '#fff1dd' },
};
...
export default function MarketDetail() {
  ...
  const tradeConfig = TRADE_TYPE_LABELS[item.tradeType] || { label: '出售', color: '#ff6b6b', bgColor: '#fff0f0' };
  ...
  const handleReport = async () => {
    try {
      ...
      await reportService.submit({ targetType: 'market', targetId: id, reason: reasons[res.tapIndex] });
    ...
    <View className='market-detail'>
      ...
      <View className='market-detail__card market-detail__price-card'>
        <View className='market-detail__price-row'>
          ...
          <View
            className='market-detail__tag'
            style={{ background: tradeConfig.bgColor }}
          >
      ...
      <View className='market-detail__card'>
        ...
        {item.description ? (
          <Text className='market-detail__desc'>{item.description}</Text>
        ) : null}
        ...
        <View className='market-detail__seller-row'>
          <View className='market-detail__avatar'>
            {item.sellerAvatarUrl ? (
              <Image className='market-detail__avatar-img' src={item.sellerAvatarUrl} mode='aspectFill' />
            ) : (
      ...
      <View className='market-detail__bottom-bar'>
        {isSold ? (
          <>
            ...
            <View className='market-detail__btn market-detail__btn--report' onClick={handleReport}>
              <Text className='market-detail__btn-text'>举报</Text>
            </View>
          </>
        ) : isSeller ? (
          <>
            <View className='market-detail__btn market-detail__btn--edit' onClick={() => Taro.navigateTo({ url: `/pages/market-edit/index?id=${id}` })}>
              <Text className='market-detail__btn-text'>编辑</Text>
            </View>
            <View className='market-detail__btn market-detail__btn--mark-sold' onClick={handleMarkSold}>
