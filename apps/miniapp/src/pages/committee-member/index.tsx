...
const CLAIM_ITEM_STATUS_CONFIG: Record<string, { label: string; color: string; bgColor: string }> = {
  ...
  approved: { label: '已通过', color: '#4CAF82', bgColor: '#E9FFF4' },
...
export default function CommitteeMember() {
  ...
  const { data: member, loading, error, refresh } = useRequest<CommitteeMemberDetailDto>(
    () => committeeService.getMemberDetail(id!),
    [id],
    { enabled: !!id },
  );
      ...
      <ScrollView scrollY className='committee-member__scroll'>
        ...
        <View className='committee-member__avatar-section'>
          <View className='committee-member__avatar'>
            {member.avatarUrl ? (
              <Image className='committee-member__avatar-img' src={member.avatarUrl} mode='aspectFill' />
            ) : (
          ...
          {member.termStart && (
            <View className='committee-member__info-row'>
              ...
              <Text className='committee-member__info-value'>
                {formatDate(member.termStart)} ~ {member.termEnd ? formatDate(member.termEnd) : '至今'}
              </Text>
            </View>
          )}
        </View>
        ...
        {member.claimStatus === ClaimStatus.UNCLAIMED && (
          <View className='committee-member__claim-section'>
            <View
              className='committee-member__claim-btn'
              onClick={handleClaim}
            >
        ...
        {member.claims && member.claims.length > 0 && (
          <View className='committee-member__claims'>
            ...
            {member.claims.map((claim) => {
              const claimStatusConfig = CLAIM_ITEM_STATUS_CONFIG[claim.status] ?? CLAIM_ITEM_STATUS_CONFIG.pending;
              return (
                <View key={claim.id} className='committee-member__claim-item'>
                  <View className='committee-member__claim-item-top'>
                    <Text className='committee-member__claim-item-statement'>{claim.statement}</Text>
                    <View
                      className='committee-member__claim-item-tag'
                      style={{ backgroundColor: claimStatusConfig.bgColor }}
                    >
                      <Text className='committee-member__claim-item-tag-text' style={{ color: claimStatusConfig.color }}>
                        {claimStatusConfig.label}
                      </Text>
                    </View>
                  </View>
                  <Text className='committee-member__claim-item-date'>{formatDate(claim.createdAt)}</Text>
                </View>
              );
            })}
          </View>
