...
export default function VoteDetail() {
  ...
  const { data: vote, loading, error, refresh } = useRequest<VoteDto>(
    ...
    <View className='vote-detail'>
      <ScrollView scrollY className='vote-detail__scroll'>
        <View className='vote-detail__tags'>
          ...
          <View className={`vote-detail__tag ${active ? 'vote-detail__tag--active' : 'vote-detail__tag--ended'}`}>
            <Text className='vote-detail__tag-text'>
              {active ? '进行中' : '已结束'}
            </Text>
          </View>
        ...
        {vote.description && (
          <Text className='vote-detail__description'>{vote.description}</Text>
        )}

        <View className='vote-detail__time'>
          <Text className='vote-detail__time-text'>
            {formatDate(vote.startAt)} ~ {formatDate(vote.endAt)}
          </Text>
        </View>

        <View className='vote-detail__options'>
          <Text className='vote-detail__options-header'>
            {vote.voteType === VoteType.SINGLE ? '请选择一项' : `请选择（最多${vote.maxChoices ?? vote.options.length}项）`}
          </Text>
          ...
          {vote.options.map((option) => {
            ...
            return (
              ...
              >
                <View className={`vote-detail__indicator ${vote.voteType === VoteType.SINGLE ? 'vote-detail__indicator--radio' : 'vote-detail__indicator--checkbox'} ${isSelected ? 'vote-detail__indicator--checked' : ''}`}>
        ...
        {showResults && results && (
          <View className='vote-detail__results'>
